...
(function () {
    ...
    var callback,
        ...
        createType = function (type, customFunctions) {
            var standardType = {
                ...
                getDisplayHtml : function (value, that) {
                    return that.escapeHtml(value);
                },
    ...
    Interface = {
        'implements' : function(obj, functions) {
            var i;
            
            for(i = 0; i < functions.length; i += 1) {
                if ("function" !== typeof obj[functions[i]]) {
                    return false;
                }
            }
    ...
    TypeFactory.prototype = {
        ...
        'addType' : function (name, obj) {
            if (Interface.implements(obj, [
                'parseValueOfType', 
                'getHtmlFormValue', 
                'generateInputField', 
                'checkType', 
        ...
        'file' : createType('file',  {
            generateInputField : function (defObj, value, td , that) {
                ...
                $(td).html(
                    '<button class="crud-gui-file-upload">Upload File</button><input type="hidden" '
                    + '" value="' + value +  '" />'
                );
            },
            checkType : function (value) {
    ...
    CRUDGenerator.prototype = {
        ...
        recognizeFieldDefinition : function (def) {
            ...
            isRequired = this.isDefinitionRequired(definitions[0]);
            if (!isRequired) {
                definitions[0] = definitions[0].substr(1, definitions[0].length);
            }
        ...
        valueIsValid : function (schemaDef, value) {
            ...
            this.typeFactory
                ...
                .checkType(value, throwMeteorError);
