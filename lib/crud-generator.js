(function () {
    // TODO: Validate, maybe javascript html5 api?
    ...
    var callback,
        ...
        checkUserRights = function (users) {
            ...
            // Never let anonymous edit stuff
            ...
            if (!_.contains(users, currentUser.username)) {
                throwMeteorError("You are not privileged to execute this CRUD operation!");
            }
    ...
    CRUDMarkupGenerator = function (options) {
        _.isUndefined(_.isUndefined(options)) ? options = { 'styling' : 'pure-css' } : '';
        _.isUndefined(options.additionalClasses) ? options.additionalClasses = '' : '';
        options.tableClass = '';

        this.options = options;

        if (!loadedFiles && !_.isUndefined(options.styling) && Meteor.isClient) {
            switch (options.styling) {
                case 'pure-css' :
                loadFile('http://yui.yahooapis.com/pure/0.2.1/pure-min.css', 'css');
    ...
    CRUDMarkupGenerator.prototype = {
        ...
        generateEditForm : function (tableRow, schema, crudGeneratorThis) {
            ...
            tableRow.children(':not(.crud-gui-button)').each(function () {
                ...
                $(this).html('<input type="' + inputType + '" value="' + that.escapeHtml(tdValue) +'" />');
            });
        },
        /**
        ...
        generateOverviewTable : function (collection, schema, name) {
            return new Handlebars.SafeString(
                '<table class="' + this.options.tableClass + ' ' + this.options.additionalClasses + ' crud-gui-' + name + '">'
