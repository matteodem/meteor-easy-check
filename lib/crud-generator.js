        ...
        generateOverviewTable : function (collection, schema, name) {
            return new Handlebars.SafeString(
                '<form class="' + this.options.formClass +'">'
                    ...
                    + '</table>' +
                '</form>' + this.generateCreateButton(name));
        }
    };
            ...
            editClickHandler : function (target) {
                ...
                switch(currentState) {
                    case 'really-edit':
                        ...
                        break;
                    case 'editable' :
                        var valid = this.checkValidityOfForm($(target).parent().parent());
                        if (valid) {
                            $(target)
                                .text('Really edit?')
    ...
    CRUDGenerator.prototype = {
        ...
        checkValidityOfForm : function (tableRow) {
            ...
            tableRow.find('input').each(function () {
                validInputs[$(this).parent().attr('data-key')] = this.checkValidity();
            });

            console.log(_.has(validInputs, false), validInputs);
            
            return false;
            
            if (_.has(validInputs, false)) {
                return false
            }
