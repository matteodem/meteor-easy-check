(function (undefined) {
    ...
    var CRUDGenerator,
        ...
        checkUserRights = function (users) {
            ...
            if (!_.contains(users, currentUser.username)) {
                throw new Meteor.Error("You are not privileged to execute this CRUD operation!");
            }
    ...
    CRUDGenerator = function (collection, schema, options) {
        ...
        // Create safe CRUD methods, if non existent
        ...
        crudFunctions[name + '_update'] = function (selector, modifier, options, callback) {
            ...
            _.each(oldDocs, function (oldDoc) {
                ...
                if (!that.validCRUDObject(newDoc, schema)) {
                    ...
                    throw new Meteor.Error("Update Operation reverted, the new document doesn't match the schema definition!");
                }
            });
        },
    ...
    CRUDGenerator.prototype = {
        ...
        validCRUDObject : function (obj, schema) {
            ...
            if (!_.isObject(obj)) {
                throw new Meteor.Error('Value to be inserted / updated needs to be an Object!');
            }

            if (objectSize(obj) !== objectSize(schema)) {
                throw new Meteor.Error('Object to be inserted needs to have the same length as the schema definition!');
            }
                ...
                switch (i) {
                    case 0:
                        if (!_.contains(contentTypes, definitions[0])) {
                            throw new Meteor.Error('First definition of "' + def + '" has to be a valid content type!');
                        }
                    ...
                    case 3:
                    default:
                        throw new Meteor.Error('Schema definitions can\'t have more than 3 defined properties, as in: "' + def + '"');
                        break;
                }
            }
        ...
        valueIsValid : function (schemaDef, value) {
            ...
            switch (definitionObj.contentType) {
                case 'date' :
                    if (!(value instanceof Date)) {
                        throw new Meteor.Error('The value "' + value + '" doesn\'t match the defined content type: date');
                    }
                    break;
                default:
                case 'array' :
            ...
            if (_.isUndefined(definitionObj.defLength) || definitionObj.defLength == -1) {
                ...
                throw new Meteor.Error ('The value "' + value.toString() + '" provided is too long!');
